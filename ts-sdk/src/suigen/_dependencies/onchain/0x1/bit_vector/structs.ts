...
export function isBitVector(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V16}::bit_vector::BitVector`; }
...
export class BitVector implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V16}::bit_vector::BitVector`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = BitVector.$typeName; readonly $fullTypeName: `${typeof PKG_V16}::bit_vector::BitVector`; readonly $typeArgs: []; readonly $isPhantom = BitVector.$isPhantom;

 readonly length: ToField<"u64">; readonly bitField: ToField<Vector<"bool">>

 private constructor(typeArgs: [], fields: BitVectorFields, ) { this.$fullTypeName = composeSuiType( BitVector.$typeName, ...typeArgs ) as `${typeof PKG_V16}::bit_vector::BitVector`; this.$typeArgs = typeArgs;

 this.length = fields.length;; this.bitField = fields.bitField; }

 static reified( ): BitVectorReified { return { typeName: BitVector.$typeName, fullTypeName: composeSuiType( BitVector.$typeName, ...[] ) as `${typeof PKG_V16}::bit_vector::BitVector`, typeArgs: [ ] as [], isPhantom: BitVector.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => BitVector.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => BitVector.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => BitVector.fromBcs( data, ), bcs: BitVector.bcs, fromJSONField: (field: any) => BitVector.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => BitVector.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => BitVector.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => BitVector.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => BitVector.fetch( client, id, ), new: ( fields: BitVectorFields, ) => { return new BitVector( [], fields ) }, kind: "StructClassReified", } }
