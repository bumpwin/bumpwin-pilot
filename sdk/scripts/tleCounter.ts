...

{
  ...
  console.log(`Encrypted object size: ${prettyBytes(Buffer.from(encryptedObject).length)}`);
...
}
{
  ...
  const idBytes2 = bcs.u64().serialize(SHARED_BOARD.unlockTimestampMs).toBytes();

  // === seal_approve トランザクションを構築 ===
  const approvalTx = new Transaction();
  approvalTx.moveCall({
    target: `${PACKAGE_ID}::${MODULE_NAME}::seal_approve`,
    arguments: [approvalTx.pure.vector('u8', Array.from(idBytes2)), approvalTx.object(SUI_CLOCK_OBJECT_ID)],
  });
  ...
  while (true) {
    ...
    try {
      ...
      console.log('Current time before decrypt:', Date.now(), new Date().toISOString());
      ...
      console.log(`Transaction executed successfully (Alice signed, Bob executed): ${JSON.stringify(result.effects)}`);
      break;
    } catch (error) {
      console.log(`Current time: ${Date.now()} (${new Date().toISOString()})`);
