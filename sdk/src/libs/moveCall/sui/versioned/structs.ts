...
export function isVersionChangeCap(type: string): boolean {
  type = compressSuiType(type);
  return type === `${PKG_V1}::versioned::VersionChangeCap`;
}
...
export class VersionChangeCap implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::versioned::VersionChangeCap`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = VersionChangeCap.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::versioned::VersionChangeCap`;
  readonly $typeArgs: [];
