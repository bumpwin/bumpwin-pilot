...
export class Referent<T extends TypeArgument> implements StructClass {
  ...
  readonly $fullTypeName: `${typeof PKG_V1}::borrow::Referent<${ToTypeStr<T>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<T>], fields: ReferentFields<T>) {
    this.$fullTypeName = composeSuiType(
      Referent.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::borrow::Referent<${ToTypeStr<T>}>`;
  ...
  static reified<T extends Reified<TypeArgument, any>>(T: T): ReferentReified<ToTypeArgument<T>> {
    return {
      ...
      fullTypeName: composeSuiType(
        Referent.$typeName,
        ...[extractType(T)]
      ) as `${typeof PKG_V1}::borrow::Referent<${ToTypeStr<ToTypeArgument<T>>}>`,
