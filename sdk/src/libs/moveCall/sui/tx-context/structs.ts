...
export function isTxContext(type: string): boolean {
  ...
  return type === `${PKG_V1}::tx_context::TxContext`;
}
...
export class TxContext implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::tx_context::TxContext`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = TxContext.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::tx_context::TxContext`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: TxContextFields) {
    this.$fullTypeName = composeSuiType(
      TxContext.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::tx_context::TxContext`;
