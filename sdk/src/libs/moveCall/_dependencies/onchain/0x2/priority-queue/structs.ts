...
export function isPriorityQueue(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V1}::priority_queue::PriorityQueue` + '<');
}
...
export class PriorityQueue<T0 extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::priority_queue::PriorityQueue`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [false] as const;

  readonly $typeName = PriorityQueue.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::priority_queue::PriorityQueue<${ToTypeStr<T0>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<T0>], fields: PriorityQueueFields<T0>) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs
    ) as `${typeof PKG_V1}::priority_queue::PriorityQueue<${ToTypeStr<T0>}>`;
  ...
  static reified<T0 extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        PriorityQueue.$typeName,
        ...[extractType(T0)]
      ) as `${typeof PKG_V1}::priority_queue::PriorityQueue<${ToTypeStr<ToTypeArgument<T0>>}>`,
...
export function isEntry(type: string): boolean {
  type = compressSuiType(type);
  return type.startsWith(`${PKG_V1}::priority_queue::Entry` + '<');
}
...
export class Entry<T0 extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::priority_queue::Entry`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [false] as const;

  readonly $typeName = Entry.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::priority_queue::Entry<${ToTypeStr<T0>}>`;
  readonly $typeArgs: [ToTypeStr<T0>];
  ...
  private constructor(typeArgs: [ToTypeStr<T0>], fields: EntryFields<T0>) {
    this.$fullTypeName = composeSuiType(
      Entry.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::priority_queue::Entry<${ToTypeStr<T0>}>`;
    this.$typeArgs = typeArgs;
  ...
  static reified<T0 extends Reified<TypeArgument, any>>(T0: T0): EntryReified<ToTypeArgument<T0>> {
    return {
      ...
      fullTypeName: composeSuiType(
        Entry.$typeName,
        ...[extractType(T0)]
      ) as `${typeof PKG_V1}::priority_queue::Entry<${ToTypeStr<ToTypeArgument<T0>>}>`,
      typeArgs: [extractType(T0)] as [ToTypeStr<ToTypeArgument<T0>>],
