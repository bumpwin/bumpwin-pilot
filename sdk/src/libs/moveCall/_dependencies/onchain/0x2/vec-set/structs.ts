...
export function isVecSet(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V1}::vec_set::VecSet` + '<');
}
...
export class VecSet<T0 extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::vec_set::VecSet`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [false] as const;

  readonly $typeName = VecSet.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::vec_set::VecSet<${ToTypeStr<T0>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<T0>], fields: VecSetFields<T0>) {
    this.$fullTypeName = composeSuiType(
      VecSet.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::vec_set::VecSet<${ToTypeStr<T0>}>`;
  ...
  static reified<T0 extends Reified<TypeArgument, any>>(T0: T0): VecSetReified<ToTypeArgument<T0>> {
    return {
      ...
      fullTypeName: composeSuiType(
        VecSet.$typeName,
        ...[extractType(T0)]
      ) as `${typeof PKG_V1}::vec_set::VecSet<${ToTypeStr<ToTypeArgument<T0>>}>`,
