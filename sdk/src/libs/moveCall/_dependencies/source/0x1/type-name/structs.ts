...
export function isTypeName(type: string): boolean {
  ...
  return type === `${PKG_V1}::type_name::TypeName`;
}
...
export class TypeName implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::type_name::TypeName`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = TypeName.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::type_name::TypeName`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: TypeNameFields) {
    this.$fullTypeName = composeSuiType(
      TypeName.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::type_name::TypeName`;
