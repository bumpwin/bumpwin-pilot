...
export function isWrapper(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V1}::dynamic_object_field::Wrapper` + '<');
}
...
export class Wrapper<Name extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::dynamic_object_field::Wrapper`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [false] as const;

  readonly $typeName = Wrapper.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::dynamic_object_field::Wrapper<${ToTypeStr<Name>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<Name>], fields: WrapperFields<Name>) {
    this.$fullTypeName = composeSuiType(
      Wrapper.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::dynamic_object_field::Wrapper<${ToTypeStr<Name>}>`;
  ...
  static reified<Name extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        Wrapper.$typeName,
        ...[extractType(Name)]
      ) as `${typeof PKG_V1}::dynamic_object_field::Wrapper<${ToTypeStr<ToTypeArgument<Name>>}>`,
