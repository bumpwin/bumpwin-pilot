...
export class VecSet<K extends TypeArgument> implements StructClass {
  ...
  readonly $fullTypeName: `${typeof PKG_V1}::vec_set::VecSet<${ToTypeStr<K>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<K>], fields: VecSetFields<K>) {
    this.$fullTypeName = composeSuiType(
      VecSet.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::vec_set::VecSet<${ToTypeStr<K>}>`;
  ...
  static reified<K extends Reified<TypeArgument, any>>(K: K): VecSetReified<ToTypeArgument<K>> {
    return {
      ...
      fullTypeName: composeSuiType(
        VecSet.$typeName,
        ...[extractType(K)]
      ) as `${typeof PKG_V1}::vec_set::VecSet<${ToTypeStr<ToTypeArgument<K>>}>`,
