...
export function isClock(type: string): boolean {
  ...
  return type === `${PKG_V1}::clock::Clock`;
}
...
export class Clock implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::clock::Clock`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = Clock.$typeName;
  readonly $fullTypeName: `${typeof PKG_V1}::clock::Clock`;
  readonly $typeArgs: [];
  ...
  static reified(): ClockReified {
    return {
      ...
      fullTypeName: composeSuiType(Clock.$typeName, ...[]) as `${typeof PKG_V1}::clock::Clock`,
      typeArgs: [] as [],
