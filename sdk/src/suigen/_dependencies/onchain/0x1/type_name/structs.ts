...
export function isTypeName(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V16}::type_name::TypeName`; }
...
export class TypeName implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V16}::type_name::TypeName`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = TypeName.$typeName; readonly $fullTypeName: `${typeof PKG_V16}::type_name::TypeName`; readonly $typeArgs: []; readonly $isPhantom = TypeName.$isPhantom;

 readonly name: ToField<String>

 private constructor(typeArgs: [], fields: TypeNameFields, ) { this.$fullTypeName = composeSuiType( TypeName.$typeName, ...typeArgs ) as `${typeof PKG_V16}::type_name::TypeName`; this.$typeArgs = typeArgs;

 this.name = fields.name; }

 static reified( ): TypeNameReified { return { typeName: TypeName.$typeName, fullTypeName: composeSuiType( TypeName.$typeName, ...[] ) as `${typeof PKG_V16}::type_name::TypeName`, typeArgs: [ ] as [], isPhantom: TypeName.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => TypeName.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => TypeName.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => TypeName.fromBcs( data, ), bcs: TypeName.bcs, fromJSONField: (field: any) => TypeName.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => TypeName.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => TypeName.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => TypeName.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => TypeName.fetch( client, id, ), new: ( fields: TypeNameFields, ) => { return new TypeName( [], fields ) }, kind: "StructClassReified", } }
