...
export function isOption(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V1}::option::Option` + '<');
}
...
export class Option<Element extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V1}::option::Option`;
  ...
  readonly $fullTypeName: `${typeof PKG_V1}::option::Option<${ToTypeStr<Element>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<Element>], fields: OptionFields<Element>) {
    this.$fullTypeName = composeSuiType(
      Option.$typeName,
      ...typeArgs
    ) as `${typeof PKG_V1}::option::Option<${ToTypeStr<Element>}>`;
  ...
  static reified<Element extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        Option.$typeName,
        ...[extractType(Element)]
      ) as `${typeof PKG_V1}::option::Option<${ToTypeStr<ToTypeArgument<Element>>}>`,
