...
export function isBag(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V30}::bag::Bag`; }
...
export class Bag implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V30}::bag::Bag`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Bag.$typeName; readonly $fullTypeName: `${typeof PKG_V30}::bag::Bag`; readonly $typeArgs: []; readonly $isPhantom = Bag.$isPhantom;

 readonly id: ToField<UID>; readonly size: ToField<"u64">

 private constructor(typeArgs: [], fields: BagFields, ) { this.$fullTypeName = composeSuiType( Bag.$typeName, ...typeArgs ) as `${typeof PKG_V30}::bag::Bag`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.size = fields.size; }

 static reified( ): BagReified { return { typeName: Bag.$typeName, fullTypeName: composeSuiType( Bag.$typeName, ...[] ) as `${typeof PKG_V30}::bag::Bag`, typeArgs: [ ] as [], isPhantom: Bag.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Bag.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Bag.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Bag.fromBcs( data, ), bcs: Bag.bcs, fromJSONField: (field: any) => Bag.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Bag.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Bag.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Bag.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Bag.fetch( client, id, ), new: ( fields: BagFields, ) => { return new Bag( [], fields ) }, kind: "StructClassReified", } }
