...
export function isTxContext(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V30}::tx_context::TxContext`; }
...
export class TxContext implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V30}::tx_context::TxContext`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = TxContext.$typeName; readonly $fullTypeName: `${typeof PKG_V30}::tx_context::TxContext`; readonly $typeArgs: []; readonly $isPhantom = TxContext.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: TxContextFields, ) { this.$fullTypeName = composeSuiType( TxContext.$typeName, ...typeArgs ) as `${typeof PKG_V30}::tx_context::TxContext`; this.$typeArgs = typeArgs;
