...
export function isID(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V30}::object::ID`; }
...
export class ID implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V30}::object::ID`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = ID.$typeName; readonly $fullTypeName: `${typeof PKG_V30}::object::ID`; readonly $typeArgs: []; readonly $isPhantom = ID.$isPhantom;

 readonly bytes: ToField<"address">

 private constructor(typeArgs: [], fields: IDFields, ) { this.$fullTypeName = composeSuiType( ID.$typeName, ...typeArgs ) as `${typeof PKG_V30}::object::ID`; this.$typeArgs = typeArgs;

 this.bytes = fields.bytes; }

 static reified( ): IDReified { return { typeName: ID.$typeName, fullTypeName: composeSuiType( ID.$typeName, ...[] ) as `${typeof PKG_V30}::object::ID`, typeArgs: [ ] as [], isPhantom: ID.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ID.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ID.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ID.fromBcs( data, ), bcs: ID.bcs, fromJSONField: (field: any) => ID.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ID.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => ID.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => ID.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => ID.fetch( client, id, ), new: ( fields: IDFields, ) => { return new ID( [], fields ) }, kind: "StructClassReified", } }
...
export function isUID(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V30}::object::UID`; }
