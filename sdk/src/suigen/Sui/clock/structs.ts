...
export function isClock(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V30}::clock::Clock`; }
...
export class Clock implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V30}::clock::Clock`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Clock.$typeName; readonly $fullTypeName: `${typeof PKG_V30}::clock::Clock`; readonly $typeArgs: []; readonly $isPhantom = Clock.$isPhantom;

 readonly id: ToField<UID>; readonly timestampMs: ToField<"u64">

 private constructor(typeArgs: [], fields: ClockFields, ) { this.$fullTypeName = composeSuiType( Clock.$typeName, ...typeArgs ) as `${typeof PKG_V30}::clock::Clock`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.timestampMs = fields.timestampMs; }

 static reified( ): ClockReified { return { typeName: Clock.$typeName, fullTypeName: composeSuiType( Clock.$typeName, ...[] ) as `${typeof PKG_V30}::clock::Clock`, typeArgs: [ ] as [], isPhantom: Clock.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Clock.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Clock.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Clock.fromBcs( data, ), bcs: Clock.bcs, fromJSONField: (field: any) => Clock.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Clock.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Clock.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Clock.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Clock.fetch( client, id, ), new: ( fields: ClockFields, ) => { return new Clock( [], fields ) }, kind: "StructClassReified", } }
